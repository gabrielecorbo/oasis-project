...
def neighbors(rows,colums,index):
    ...
    return v,v2
# %%
def exagon(r,y_lim,x_lim,traffic_points_gdf):
    xmin =x_lim[0]
    xmax =x_lim[1]
    ymin =y_lim[0]
    ymax =y_lim[1]
...
#%%
def gen_parameters(df_demand, df_parking, flow_traffic, poi_gdf, vec_poi):
...
#%%
def optimize(df_demand, df_parking, number, flow_traffic, poi_gdf, vec_poi, pen_rate, catch_rate, avg_cap, wrk_hours, crg_rate, rows, cols, car_park_df):

    # Import i and j set function
    demand_lc, chg_lc = gen_sets(df_demand, df_parking)

    # Import parameters function
    di, fti, distance_matrix, d_poi_scale = gen_parameters(df_demand, df_parking, flow_traffic, poi_gdf, vec_poi)
...
def rotterdam_function(N):
    
    local_path = '\\rotterdam'
    ...
    y_lim = (51.86,51.95)                                    # y coordinates (boundaries of city of Rotterdam)
    x_lim = (4.42,4.57)                                    # x coordinates (boundaries of city of Rotterdam)
    ...
    polygons,rows,cols,colore,tot_traffic = exagon(radius,y_lim,x_lim,traffic_points_gdf)
    ...
    opt_loc, stat, opt_loc_df, opt_loc_df2, color_opt = optimize(GIS_df,car_park_df,number, flow_traffic, poi_gdf, vec_poi, pen_rate, catch_rate, avg_cap, wrk_hours, crg_rate, rows, cols, car_park_df)
