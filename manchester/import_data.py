...
# %%
#locate traffic data
data_path = os.getcwd()+'\\csv_files\\traffic_data.csv'
...
data = pd.read_csv(data_path)
df_charg_stat = pd.DataFrame(data=data)
df_charg_stat = df_charg_stat[['latitude','longitude']]
# %%
...
#print(poi_gdf)
#coll=['osm_id','code','fclass','coords','longitude','latitude']
coll=['coords']
for col in coll:
    #print(col)
...
poi_df_clip['longitude']=poi_gdf['longitude']
poi_df_clip['latitude']=poi_gdf['latitude']
...
base.set_xlim(x_lim)
base.set_ylim(y_lim)
traffic_points_clip.plot(ax=base,column='all_motor_vehicles',legend=True)#, cmap='cool')
# %%
...
def exagon(r,y_lim,x_lim):
    ...
    for x in np.arange(xmin, xmax, h):
        ...
        for y in np.arange(ymin, ymax, (h * h / r / 2)):
            ...
            chargers=existing_chargers_gdf.clip(hexagon)['longitude'].count()
